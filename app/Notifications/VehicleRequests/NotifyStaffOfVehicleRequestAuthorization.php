...
class NotifyStaffOfVehicleRequestAuthorization extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $url = $vehicleRequest->request_type === 'TO' ? url('/travel-requests/'.$vehicleRequest->id.'#trip-ticket') : url('/vehicle-requests/'.$vehicleRequest->id.'#trip-ticket');

        try {
            $mail = (new MailMessage)
                    ->subject('(DEPDev RO1 HRIS) Use of Official Vehicle Granted to Vehicle Request No. '.$vehicleRequest->reference_no)
                    ->markdown('emails.vehicle-requests.authorized', [
                        'order' => $order,
                        'url' => $url,
                    ]);
