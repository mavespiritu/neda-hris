...
class NotifyReviewerOfVehicleRequest extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        try {
            $mail = (new MailMessage)
                    ->subject('(DEPDev RO1 HRIS) Review of Vehicle Request No. '.$vehicleRequest->reference_no)
                    ->markdown('emails.vehicle-requests.review', [
                        'order' => $order,
                        'url' => $url,
                    ]);
