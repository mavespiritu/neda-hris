...
class NotifyAuthorizerOfVehicleRequest extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $authorizeUrl = URL::temporarySignedRoute(
            'vehicle-requests.authorize.email',
            now()->addMonth(),
            ['token' => $token]
        );
        ...
        try {
            $mail = (new MailMessage)
                    ->subject('(DEPDev RO1 HRIS) Authorization of Official Vehicle for Vehicle Request No. '.$vehicleRequest->reference_no)
                    ->markdown('emails.vehicle-requests.authorize', [
                        'order' => $order,
