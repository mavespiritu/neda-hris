...
class NotifyApproverOfVehicleRequest extends Notification implements ShouldQueue
{
    use Queueable;
    ...
    public function toMail(object $notifiable): MailMessage
    {
        ...
        $approveUrl = URL::temporarySignedRoute(
            'vehicle-requests.approve.email',
            now()->addMonth(),
            ['token' => $token]
        );
        ...
        try {
            $mail = (new MailMessage)
                    ->subject('(DEPDev RO1 HRIS) Approval of Vehicle Request No. '.$vehicleRequest->reference_no)
                    ->markdown('emails.vehicle-requests.approve', [
                        'order' => $order,
