...
namespace App\Actions\Performance\Mfo;
...
class ManageMfos
{
    ...
    public function store(Request $request)
    {
        ...
        $data = $request->validate([
            'mfo' => ['required', 'string', 'max:255'],
            'description' => ['nullable', 'string'],
        ]);

        $exists = PerformanceLibraryItem::query()
            ->where('item_type', self::ITEM_TYPE)
            ->whereRaw('LOWER(TRIM(title)) = ?', [strtolower(trim($data['mfo']))])
            ->exists();

        if ($exists) {
            return response()->json([
                'message' => 'This MFO already exists.',
                'errors' => [
                    'mfo' => ['This MFO already exists.'],
                ],
            ], 422);
        }
        ...
        return response()->json(['message' => 'MFO saved successfully.']);
    }
    ...
    public function update(Request $request, int $id)
    {
        ...
        return response()->json(['message' => 'MFO updated successfully.']);
    }
    ...
    public function destroy(int $id)
    {
        ...
        return response()->json(['message' => 'MFO deleted successfully.']);
    }
    ...
    public function bulkDestroy(Request $request)
    {
        ...
        return response()->json(['message' => 'Selected MFOs deleted successfully.']);
    }
}
