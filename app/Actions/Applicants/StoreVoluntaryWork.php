...
class StoreVoluntaryWork
{
    ...
    public function withValidator($validator, ActionRequest $request): void
    {
        $validator->after(function ($validator) use ($request) {
            ...
            if (! $isPresent && blank($data['hours'] ?? null)) {
                $validator->errors()->add('hours', 'The number of hours is required when this is not the present work.');
            }
        });
    }
    ...
    public function asController(ActionRequest $request, int $applicantId)
    {
        try {
            ...
            $id = $conn->table('applicant_voluntary_work')->insertGetId([
            ...
            return response()->json(['status' => 'success','title' => 'Success!','message' => 'Voluntary work added successfully.','id' => $id]);
        } catch (\Throwable $e) {
            Log::error('Failed to store applicant voluntary work: ' . $e->getMessage());
