...
const OtherEvidenceForm = ({ open, onClose }) => {
    ...
    setData(prev => {
        ...
        if (selectedEvidence && selectedEvidenceData.data) {
            updatedData = {
                ...
                title: selectedEvidenceData.data.title ?? "",
                description: selectedEvidenceData.data.description ?? "",
                start_date: selectedEvidenceData.data.start_date ?? "",
                end_date: selectedEvidenceData.data.end_date ?? "",
  ...
  return (
    <Dialog open={open} onOpenChange={onClose}>
        <DialogContent>
          {(performances.isLoading || selectedEvidenceData.isLoading) && (
              <div className="absolute inset-0 z-50 flex items-center justify-center backdrop-blur-xs bg-black/30 rounded-lg">
                  <Loader2 className="h-10 w-10 animate-spin text-white" />
              </div>
          )}
          <DialogHeader>
              <DialogTitle>Add other type of evidence</DialogTitle>
              <DialogDescription className="text-justify">
              Add other type of evidence and give context for this indicator: 
              <br/>
          ...
          <form onSubmit={handleSubmit} className="flex flex-col gap-4">
            <div className="flex flex-col gap-2">
              <Label htmlFor="title">Title of Evidence</Label>
              <TextInput 
                name="title" 
