...
const AttachWorkExperienceForm = ({ open, onClose, jobId, applicantId, requirement }) => {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="flex flex-col text-sm font-medium border rounded-lg px-4 py-2">
              <span>(a) {requirement?.subReq?.agency}</span>
              <span>(b) {requirement?.subReq?.position}</span>
              <span>(c) P{formatNumberWithCommas(requirement?.subReq?.monthly_salary ?? 0)}</span>
            </p>
          </div>
...
export default AttachWorkExperienceForm
