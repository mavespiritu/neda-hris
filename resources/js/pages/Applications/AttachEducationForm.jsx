...
const AttachEducationForm = ({ open, onClose, jobId, applicantId, requirement }) => {
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex flex-col gap-2">
            ...
            <p className="flex flex-col text-sm font-medium border rounded-lg px-4 py-2">
              <span>(a) {requirement?.subReq?.level}</span>
              <span>(b) {requirement?.subReq?.school}</span>
              <span>(c) {requirement?.subReq?.course}</span>
            </p>
          </div>
...
export default AttachEducationForm
