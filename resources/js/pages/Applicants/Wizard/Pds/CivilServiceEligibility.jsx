...
const CivilServiceEligibility = ({
  ...
  const openAddDialog = async () => {
    ...
    try {
      const response = await axios.get(
        route("applicants.create-civil-service-eligibility", { applicantId })
      )
  ...
  const openEditDialog = async (row) => {
    ...
    try {
      const response = await axios.get(
        route("applicants.edit-civil-service-eligibility", { applicantId, id: row.id })
      )
      setFormData(response.data)
    } catch {
      setDialogOpen(false)
  ...
  const confirmDelete = async () => {
    ...
    try {
      await axios.delete(
        route("applicants.delete-civil-service-eligibility", { applicantId, id: recordToDelete.id })
      )
  ...
  const handleSubmit = async () => {
    ...
    try {
      if (dialogMode === "add") {
        await axios.post(
          route("applicants.store-civil-service-eligibility", { applicantId }),
          formData
        )
      } else {
        await axios.put(
          route("applicants.update-civil-service-eligibility", { applicantId, id: formData.id }),
          formData
        )
      }
    ...
    <>
      <Card>
        <CardHeader className="flex flex-row items-start justify-between gap-4">
          ...
          <Button type="button" onClick={openAddDialog} disabled={!applicantId}>
