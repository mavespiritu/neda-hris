...
const Review = ({setCurrentStep, manualMode, action, applicantId}) => {
  ...
  useEffect(() => {
  if (manualMode) {
    if (action === 'create' && !applicantId) {
      // Scenario 1: New empty applicant
      fetchPds({ action }) // fetch empty values based on action
    } else if ((action === 'create' && applicantId) || (action === 'update' && applicantId)) {
      // Scenario 2 & 3: Existing applicant
      fetchPds({ applicantId, action })
    } else {
      // Scenario 4: fallback to logged-in user
      fetchPds({ action })
    }
  } else {
    // Logged-in user mode
      ...
      <CardContent className="flex flex-col gap-4 border-t">
        ...
        >
          <AccordionItem value="item-1">
            <AccordionTrigger>
              <span className="flex items-center">
                <UserPen className="mr-2 h-4 w-4"/>I. Personal Information
              </span>
            </AccordionTrigger>
            <AccordionContent className="flex flex-col gap-4 text-balance">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4 border rounded-lg p-4">
          ...
          <AccordionItem value="item-2">
            <AccordionTrigger>
              <span className="flex items-center">
                <House className="mr-2 h-4 w-4"/>II. Family Background
              </span>
            </AccordionTrigger>
            <AccordionContent className="flex flex-col gap-4 text-balance">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-4 border rounded-lg p-4">
          ...
          <AccordionItem value="item-3">
            <AccordionTrigger>
              <span className="flex items-center">
                <GraduationCap className="mr-2 h-4 w-4"/>III. Educational Background
              </span>
            </AccordionTrigger>
            <AccordionContent className="flex flex-col gap-4 text-balance">
                ...
                <Table>
                  ...
                  {educationalBackground.elementary?.map((child, index) => (
                    <TableRow key={index}>
                      ...
                      <TableCell className="text-center">{child.from_year}</TableCell>
                      <TableCell className="text-center">{child.to_year}</TableCell>
          ...
          <AccordionItem value="item-4">
            <AccordionTrigger>
              <span className="flex items-center">
                <FileText className="mr-2 h-4 w-4"/>IV. Civil Service Eligibility
              </span>
            </AccordionTrigger>
            <AccordionContent className="flex flex-col gap-4 text-balance">
              <div className="space-y-6 border rounded-lg mb-4">
          ...
          <AccordionItem value="item-5">
            <AccordionTrigger>
              <span className="flex items-center">
                <BriefcaseBusiness className="mr-2 h-4 w-4"/>V. Work Experience
              </span>
            </AccordionTrigger>
            <AccordionContent className="flex flex-col gap-4 text-balance">
              <div className="space-y-6 border rounded-lg mb-4">
          ...
          <AccordionItem value="item-6">
            <AccordionTrigger>
              <span className="flex text-left items-center">
                <Waypoints className="mr-2 h-4 w-4"/>VI. Voluntary Work or Involvement in Civic/Non-Government/People/Voluntary Organization/s
              </span>
            </AccordionTrigger>
            <AccordionContent className="flex flex-col gap-4 text-balance">
              <div className="space-y-6 border rounded-lg mb-4">
          ...
          <AccordionItem value="item-7">
            <AccordionTrigger>
              <span className="flex items-center">
                <Brain className="mr-2 h-4 w-4"/>VII. Learning and Development (L&D) Interventions/Training Programs Attended
              </span>
            </AccordionTrigger>
            <AccordionContent className="flex flex-col gap-4 text-balance">
              <div className="space-y-6 border rounded-lg mb-4">
                <Table>
                  ...
                  {learningAndDevelopment?.map((child, index) => (
                    <TableRow key={index}>
                      <TableCell>{index+1}</TableCell>
                      <TableCell>{child.seminar_title}</TableCell>
                      <TableCell className="text-center">{formatDate(child.from_date)}</TableCell>
          ...
          <AccordionItem value="item-8">
            <AccordionTrigger>
              <span className="flex items-center">
                <SquareLibrary className="mr-2 h-4 w-4"/>VIII. Other Information
              </span>
            </AccordionTrigger>
            <AccordionContent className="flex flex-col gap-4 text-balance">
                        ...
                        <TableRow key={`question-${index}`} className="bg-muted/10">
                          ...
                          <TableCell>{question.answer ? (
                            <div className="flex flex-col gap-2">
                              ...
                              <div>{question.question_details}: 
                                {question.answer === 'yes' && <span className="underline">{question.details}</span>}
                              </div>
                            </div>
                          ) : ""}</TableCell>
                        </TableRow>
                        ...
                        {question.subQuestions?.map((subQuestion, subIndex) => (
                          <TableRow key={`subquestion-${index}-${subIndex}`}>
                            ...
                            <div className="flex flex-col gap-2">
                              ...
                              <div>{subQuestion.question_details}: 
                                {subQuestion.answer === 'yes' && <span className="underline">{subQuestion.details}</span>}
                              </div>
                            </div>
                          ) : ""}</TableCell>
                          </TableRow>
